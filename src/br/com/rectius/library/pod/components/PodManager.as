...
package br.com.rectius.library.pod.components
{
	...
	public class PodManager extends SkinnableContainer
	{
		...
		[ArrayElementType("br.com.rectius.library.pod.components.PodWindow")]
		...
		protected function changeHandler(event:PodWindowEvent):void
		{
			...
			var source:PodWindow = event.data.source;
			var target:PodWindow = event.data.target;
		...
		protected function restoreHandler(event:PodWindowEvent):void
		{
			...
			var pod:PodWindow = PodWindow(event.data);
		...
		protected function maximizeHandler(event:PodWindowEvent):void
		{
			...
			{
				var element:PodWindow = pod.podParent.getElementAt(index) as PodWindow;
				if(element != pod)
				{
		...
		private function changePodAspect(container:IVisualElementContainer, action:Boolean, mode:String=null):void
		{
			...
			{
				var element:PodWindow = PodWindow(container.getElementAt(index));
